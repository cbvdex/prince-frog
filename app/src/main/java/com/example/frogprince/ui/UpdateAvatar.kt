...
class UpdateAvatar : AppCompatActivity() {
    ...
    fun populateData() {
        ...
        if(avatar != null) {
            ...
            if(avatar!!.imageLink!=null && !avatar!!.imageLink.equals("")) {
                newImageLink = avatar!!.imageLink
            }

            if (manager!= null) {
                val b: Bitmap? = manager.imageDAO.getImage(avatar!!.imageLink)
                if(b != null) {
                    imgUpdate.setImageBitmap(b)
                }
            }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == SELECT_IMAGE && resultCode == Activity.RESULT_OK
            ...
            try {
                ...
                var newId: Long = manager.imageDAO.insertImage(byteArray)
                newImageLink = newId.toString()
                Utility.println("new image: " + newId)
                val b: Bitmap? = manager.imageDAO.getImage(newId.toString())
                imgUpdate.setImageBitmap(b)
    ...
    fun saveChangeOnly(view : View){
        ...
        if(avatar != null){
            ...
            val imageLink : String = if(newImageLink!=null) newImageLink!! else ""
